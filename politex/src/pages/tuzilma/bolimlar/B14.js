...
const B14 = () => {
    const { data } = useQuery({
        queryKey: ["bulim", 24],
        queryFn: () => getBulimId(24),
    });
    let datas = data?.data?.data[0];
    const { t } = useTranslation();

    document.title=t("header.header__sub__menu.menu-2.item-14")
    ...
    return (
        <>
            <Layout>
                <BigFotoBack
                    img={bg}
                    text={t("header.header__sub__menu.menu-2.item-14")}
                />

                <CustomTitle name={t("header.header__sub__menu.menu-2.item-14")} />
