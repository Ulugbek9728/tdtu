...
import bg from "@/assets/bolimlar/TTJ.jpg";

const B15 = () => {
    const { data } = useQuery({
        queryKey: ["bulim", 25],
        queryFn: () => getBulimId(25),
    });
    let datas = data?.data?.data[0];
    const { t } = useTranslation();
    document.title=t("header.header__sub__menu.menu-2.item-15")
    ...
    return (
        <>
            <Layout>
                <BigFotoBack
                    img={bg}
                    text={t("header.header__sub__menu.menu-2.item-15")}
                />

                <CustomTitle name={t("header.header__sub__menu.menu-2.item-15")} />
