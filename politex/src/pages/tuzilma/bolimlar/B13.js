...
import bg from "@/assets/bolimlar/kadr.jpg";

const B13 = () => {
    const { data } = useQuery({
        queryKey: ["bulim", 23],
        queryFn: () => getBulimId(23),
    });
    let datas = data?.data?.data[0];
    const { t } = useTranslation();
    document.title=t("header.header__sub__menu.menu-2.item-13")
    return (
        <>
            <Layout>
                <BigFotoBack
                    img={bg}
                    text={t("header.header__sub__menu.menu-2.item-13")}
                />

                <CustomTitle name={t("header.header__sub__menu.menu-2.item-13")} />
